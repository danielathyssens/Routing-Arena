...
import itertools
import numpy as np
from models.DeepACO.DeepACO.cvrp_nls.swapstar import swapstar  # models.DeepACO.DeepACO.cvrp_nls.
...
class ACO():
    ...
    @torch.no_grad()
    def run(self, n_iterations, inference=False, time_limit=None):
        # iteration_times_best = torch.zeros(size=(len(range(n_iterations)), 2))
        iteration_times_best = []
        iteration_solutions = []
        ...
        while i < n_iterations_:
            ...
            it_end_t = time.time() - start_time_it

            if time_limit is not None:
                if it_end_t >= time_limit:
                    print(f'Time Budget is up: {it_end_t} >= {time_limit}')
                    break
                elif i == n_iterations_-1:
                    n_iterations_ = n_iterations_ + 1

            # added:
            iteration_times_best.append(torch.tensor([it_end_t, self.lowest_cost]))
            iteration_solutions.append(get_subroutes(self.shortest_path))

            i += 1

        return self.lowest_cost, torch.stack(iteration_times_best, dim=0), iteration_solutions
    ...
    def pick_move(self, prev, visit_mask, capacity_mask, require_prob):
        ...
        # print('self.alpha, self.beta', self.alpha, self.beta)
        # print('visit_mask', visit_mask)
        # print('capacity_mask', capacity_mask)
