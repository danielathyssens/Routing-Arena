...
def infer_instance(model, pyg_data, distances, n_ants):
    ...
    best_aco_1, _ = aco.run(n_iterations=1, inference = True)
    # print('best_aco_1', best_aco_1)
    best_aco_T, _ = aco.run(n_iterations=T-1, inference = True)
    # print('best_aco_T', best_aco_T)
...
def train(n_node, n_ants, steps_per_epoch, epochs, k_sparse = None, batch_size = 3, test_size = None, pretrained = None, savepath = "../pretrained/tsp_nls"):
    ...
    print(f"k_sparse:{k_sparse},n_ants:{n_ants},steps_per_epoch:{steps_per_epoch},epochs:{epochs},batch_size:{batch_size}")
    net = Net().to(device)
    if pretrained:
