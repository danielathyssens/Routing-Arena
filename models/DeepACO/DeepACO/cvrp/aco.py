...
class ACO():
    ...
    @torch.no_grad()
    def run(self, n_iterations, time_limit=None):
        # iteration_times_best = torch.zeros(size=(len(range(n_iterations)), 2))
        iteration_times_best = []  # cannot do torch tensor - with time limit don't know length
        iteration_solutions = []
        ...
        while i < n_iterations_:
            ...
            # added:
            iteration_times_best.append(torch.tensor([time.time() - start_time_it, self.lowest_cost]))
