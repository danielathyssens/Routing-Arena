...
from models.HGS.HGS import eval_rp
...
class Runner:
    ...
    def __init__(self, cfg: DictConfig):
        ...
        # Model acronym
        self.acronym = 'HGS' if self.cfg.policy == 'HGS' else 'hygese'
        if cfg.explicit_name:
            self.acronym = self.get_acronym(self.acronym)
    ...
    def _build_policy(self):
        """Load and prepare data and initialize hygese routing models."""
        # Solver initialization (only called if policy == hygese)
        if self.cfg.policy == 'hygese':
            raise NotImplementedError
            # ap = AlgorithmParameters(timeLimit=self.per_instance_time_limit)  # seconds
            # self.policy = Solver(parameters=ap, verbose=True)  # hgs_solver
        else:
            self.policy = None
    ...
    def run_inference(self):
        ...
        _, solutions_ = eval_rp(policy=self.policy,
                                data=self.ds.data,
    ...
    def get_acronym(self, model_name: str):
        return model_name + "_" + self.cfg.policy.nbIter
...
def update_path(cfg: DictConfig, fixed_dataset: bool = True):
    ...
    if cfg.policy_cfg.hgs_exe_path is not None:
        cfg.policy_cfg.hgs_exe_path = os.path.normpath(
            os.path.join(cwd, cfg.policy_cfg.hgs_exe_path)
        )
