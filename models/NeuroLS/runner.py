...
class Runner(BaseSearchRunner, ABC):
    ...
    def test(self, additional_test_cfg: Optional[Union[DictConfig, Dict]] = None, **kwargs):
        ...
        self.ds = self.get_test_set(cfg=self.cfg, DATA_CLASS=DATA_CLASS, data_transform=to_RPInstance)
        # self._build_problem(compatible_problems=DATA_CLASS, data_transform=to_RPInstance)
        ...
        # ckpt_pth = self.cfg.get('checkpoint_load_path')
        ckpt_pth = self.cfg.test_cfg.checkpoint_load_path
        assert ckpt_pth is not None
        ...
        for ds in tqdm(ds_iter, disable=same_size):
            ...
            for run in range(1, number_of_runs + 1):
                ...
                solutions_ = self.run_inference_neurols(test_collector, monitor, render, ds)
    ...
    def run_inference_neurols(self, test_collector, monitor, render, dataset):
        ...
        solutions_internal, summary = self._eval(solutions_, time_limit=tl)
