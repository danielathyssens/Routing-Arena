...
from models.NeuroLS.lib.routing import RPInstance, RPSolution, RP_TYPES
from models.NeuroLS.lib.scheduling import JSSPInstance, JSSPSolution, JSSP_TYPES
from models.NeuroLS.lib.env.utils import parse_solutions
...
def parse_instances(instances: np.ndarray,
                    problem: str
                    ) -> List[Union[RPInstance, CVRPInstance, JSSPInstance]]:
    # _w = RPInstance if problem.upper() in RP_TYPES else JSSPInstance
    # _w = CVRPInstance if problem.upper() in RP_TYPES else JSSPInstance
    # instances_list = [_w(*inst.tolist()) for inst in instances]
    # return instances_list
    return [
        CVRPInstance(
            coords=instance[0],
            node_features=instance[1],
            graph_size=instance[2],
            depot_idx=instance[3],
            constraint_idx=instance[4],  # demand is at last position of node features
            vehicle_capacity=instance[5],  # demands are normalized
            max_num_vehicles=instance[6],
            instance_id=i
        )
...
def test_episode(
    ...
    running_times = info_buf.get('running_times')[dones]
    running_sols = info_buf.get('running_sols')[dones]
    final_running_sols = [[subsol for subsol in instance if subsol != [0]] for instance in running_sols]
    _running_times = running_times.tolist()
    final_running_times = [[time_ for time_ in sublist if time_ != 0] for sublist in _running_times]
    costs = info_buf.get('best_cost_final')[dones]
    result['final_costs'] = costs
    result['running_sols'] = final_running_sols
    result['running_times'] = final_running_times
    ...
    # print('solutions[-1].instance.instance_id', solutions[-1].instance.instance_id)
    # print('solutions[-1].instance.coords[:3]', solutions[-1].instance.coords[:3])
...
def offpolicy_trainer(
    ...
    :param int start_epoch: the epoch from which to resume training - Default is 0 (fresh train run)
...
def tester(
    ...
    return result, solutions, test_result['running_sols'], test_result['running_times']
...
class TestCollector(Collector):
    ...
    def reset_env(self) -> None:
        super().reset_env()
        print('self.env_num', self.env_num)
    ...
    def collect(
        ...
        print('ready_env_ids beginn', ready_env_ids)
        start_time = time.time()
        step_count = 0
        episode_count = 0
        episode_rews = []
        ...
        while True:
            ...
            print('len(whole_data', len(whole_data))
            self.data = self.data[ready_env_ids]
            self.data.obs.keys()
            print('self.data.obs.keys()', self.data.obs.keys())
            ...
            print('obs_next.size', obs_next.size)
            ...
            if np.any(done):
                ...
                # print('self.data[env_ind_global]', self.data[env_ind_global])
                ...
                # print('env_ind_local', env_ind_local)
            ...
            try:
                print('ready_env_ids', ready_env_ids)
                print('whole_data.obs[ready_env_ids]..batch_idx.shape', whole_data.obs[ready_env_ids].batch_idx.shape)
                print('self.data.obs_next.batch_idx.shape', self.data.obs_next.batch_idx.shape)
                print('whole_data.obs[ready_env_ids].node_features.shape', whole_data.obs[ready_env_ids].node_features.shape)
                print('self.data.obs_next.node_features.shape', self.data.obs_next.node_features.shape)
                print('whole_data.obs[ready_env_ids].nbh_edges.shape', whole_data.obs[ready_env_ids].nbh_edges.shape)
                print('self.data.obs_next.nbh_edges.shape', self.data.obs_next.nbh_edges.shape)
                print('whole_data.obs[ready_env_ids].nbh_weights.shape', whole_data.obs[ready_env_ids].nbh_weights.shape)
                print('self.data.obs_next.nbh_weights.shape', self.data.obs_next.nbh_weights.shape)
                print('whole_data.obs[ready_env_ids].best_sol.shape', whole_data.obs[ready_env_ids].best_sol.shape)
                print('self.data.obs_next.best_sol.shape', self.data.obs_next.best_sol.shape)
                print('whole_data.obs[ready_env_ids].current_sol.shape', whole_data.obs[ready_env_ids].current_sol.shape)
                print('self.data.obs_next.best_sol.shape', self.data.obs_next.current_sol.shape)
                print('whole_data.obs[ready_env_ids].current_sol_seq.shape', whole_data.obs[ready_env_ids].current_sol_seq.shape)
                print('self.data.obs_next.current_sol_seq.shape', self.data.obs_next.current_sol_seq.shape)
                print('whole_data.obs[ready_env_ids].meta_features.shape', whole_data.obs[ready_env_ids].meta_features.shape)
                print('self.data.obs_next.meta_features.shape', self.data.obs_next.meta_features.shape)
