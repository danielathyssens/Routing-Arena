...
from models.NeuroLS.lib.env.base_env import BaseEnv
from models.NeuroLS.lib.env.modes import *
from models.NeuroLS.lib.routing.local_search.rules import *
from models.NeuroLS.lib.routing import RPGenerator, RPDataset, ConstructionHeuristic, RPInstance
from models.NeuroLS.lib.env.utils import inverse_key_lookup, Viewer, parse_solutions
...
class VRPBaseEnv(BaseEnv):
    ...
    def __init__(self,
                 ...
                 RA_dataset_class: CVRPDataset = None,
                 **kwargs):
        ...
        self.dataset_class = RA_dataset_class
    ...
    def _get_observation(self, reset: bool = False) -> Dict:
        return {
    ...
    def _get_meta_features(self):
        mf = np.array([
            ...
            self._current_step,
            self._ls_time,
            ((default_timer() - self._tinit) * 1000) / self.time_limit if self.time_limit < np.inf else -1
        ], dtype=self.float_prec)
        # if self.position_mode == 'SELECT_SG' and 'RADIAN' in self.sg_selection_mode:
    ...
    def _step(self, action: Union[np.ndarray, List, int], **kwargs) -> Tuple[float, bool, Dict]:
        ...
        self._ls_time = (default_timer() - tstart) * 1000
        ...
        reward = self._compute_reward(self.current_cost, self.best_cost, clamp=self.clamp,
                                      time=self._ls_time if self.time_reward else None)

        if self.time_limit < np.inf:
            done = ((default_timer() - self._tinit) * 1000 >= self.time_limit - 1)
        else:
    ...
    def _init_generator(self, problem: str, **kwargs):
        ...
        self.data_generator = self.dataset_class  # dataset class given directly from runner
