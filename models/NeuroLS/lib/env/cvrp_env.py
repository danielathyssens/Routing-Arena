...
from models.NeuroLS.lib.env.rp_env import VRPBaseEnv
from models.NeuroLS.lib.routing import RPInstance, knn_nbh
from models.NeuroLS.lib.routing.local_search.rules import *
from models.NeuroLS.lib.routing.local_search import VRP
...
class CVRPEnv(VRPBaseEnv):
    ...
    def _init_instance(self, instance: RPInstance, **kwargs):
        ...
        self.max_seq_len = int(min(128, np.ceil(N*0.75)))
    ...
    def _parse_assignment(self, vrph_route: Union[np.ndarray, List, Any]):
        ...
        print('self.max_seq_len-len(r)-1', self.max_seq_len-len(vrph_route[0])-1)
        print('self.max_seq_len-len(r)-1', self.max_seq_len - len(vrph_route[1]) - 1)
        current_sol_seq = [
                self.instance.depot_idx + r + [0]*(self.max_seq_len-len(r)-1) for r in vrph_route
            ]
        print('self.max_seq_len', self.max_seq_len)
        print('self.max_num_vec', self.max_num_vec)
        print('(self.max_num_vec-len(current_sol_seq))', (self.max_num_vec-len(current_sol_seq)))
        ...
        if self.current_sol_seq.shape[0] > self.max_num_vec:
            print('self.max_num_vec', self.max_num_vec)
            print('self.current_sol_seq.shape[0]', self.current_sol_seq.shape[0])
