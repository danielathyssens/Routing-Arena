...
from models.NeuroLS.lib.routing import RPGenerator
from models.NeuroLS.lib.scheduling import JSSPGraph, JSSPGenerator
...
class DistributedDataset:
    ...
    def load(self, fpath: str, limit: Optional[int] = None, **kwargs):
        ...
        if isinstance(data, np.ndarray):
            data = data.tolist()
        elif isinstance(data, CVRPDataset):
            data = data.data
        else:
            data = data
...
def _test(
    ...
    **kwargs
    ...
    from models.NeuroLS.lib.env import TSPEnv
    np.random.seed(seed)
