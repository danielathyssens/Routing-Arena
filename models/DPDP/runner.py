...
class Runner(BaseConstructionRunner):
    ...
    def _run_model(self):
        return eval_model(ckpt_pth=self.ckpt_path,
    ...
    def test(self):
        ...
        # model-specific cfg-args for dataset-setup
        # DPDP specific dataset args
        self.make_dataset_kwargs = {'normalize': False} if self.cfg.eval_opts_cfg.decode_strategy[:4] in (
            'dpbs', 'dpdp') and self.cfg.problem.upper() == 'CVRP' else {}

        self.cfg['normalize'] = self.make_dataset_kwargs['normalize'] if self.make_dataset_kwargs else True
