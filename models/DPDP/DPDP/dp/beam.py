####
# This code file is partly changed from the original code provided in
...
# changes: 228, 326-328 (del line 4)
import numpy as np
import torch

# models.DPDP.DPDP.
from models.DPDP.DPDP.dp.potentials import update_potential_info
from models.DPDP.DPDP.utils.mask_utils import unpack_mask

from models.DPDP.DPDP.utils.sorting_utils import unique_inverse, unique1d
from models.DPDP.DPDP.utils.scatter_utils import coo_to_csr
...
def update_mask(all_actions, all_parent_beamrows, mask, mask_idx, col_idx_per_group):
    ...
    # a_col = torch.floor_divide(a, mask_bits) if len(mask) > 1 or len(mask[0]) > 1 else None # deprecated - false round
    a_col = torch.div(a, mask_bits, rounding_mode='floor') if len(mask) > 1 or len(mask[0]) > 1 else None
