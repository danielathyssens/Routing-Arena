...
from models.NeuOpt.NeuOpt.utils import clip_grad_norms
from models.NeuOpt.NeuOpt.nets.actor_network import Actor
from models.NeuOpt.NeuOpt.nets.critic_network import Critic
from models.NeuOpt.NeuOpt.utils import torch_load_cpu, get_inner_model, move_to
from models.NeuOpt.NeuOpt.utils.logger import log_to_tb_train
from models.NeuOpt.NeuOpt.agent.utils import validate,gather_tensor_and_concat
from models.NeuOpt.NeuOpt.problems.problem_cvrp import total_history
...
class PPO:
    ...
    def rollout(self, problem, T, val_m, stall_limit, batch, Time_Budget=10000, record = False,
                record_best=True, show_bar = False, t_start_=None):
        import time
        ...
        sol_times_traj = []
        cur_t = time.time() - time_start
        for i in range(bs):
            sol_times_traj.append((cur_t, solutions.clone().cpu().detach().numpy()[i][None,:]))
        ...
        ###
        sol_history = [solution_best.clone()]
        ...
        for t in tqdm(range(T), disable = self.opts.no_progress_bar or not show_bar, desc = 'rollout', bar_format='{l_bar}{bar:20}{r_bar}{bar:-20b}'):       
            ...
            if r_t >= Time_Budget:
                print(f'NeuOpt RUNTIME {r_t} REACHED TIME LIMIT {Time_Budget} --> BREAK')
                ###
            ...
            sol_history.append(solution_best)
            for i in range(bs):
                sol_times_traj.append((r_t, solutions.cpu().detach().numpy()[i][None,:]))

            if feasibility_history is not None:
