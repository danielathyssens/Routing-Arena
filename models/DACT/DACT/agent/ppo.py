...
class PPO:
    def __init__(self, problem_name, size, opts):
        ...
        self.actor = Actor(
            ...
            device=torch.device("cuda" if opts.use_cuda and torch.cuda.is_available() else "cpu")
        )
    ...
    def rollout(self, problem, val_m, batch, Time_Budget=10000, do_sample=False, record=False, record_best=True,
                show_bar=False, t_start_=None):
        ...
        # while time.time() < (Time_Budget - time_start):
        for t in tqdm(range(self.opts.T_max), disable=self.opts.no_progress_bar or not show_bar,
            ...
            # pass through model
            # print('next(problem.parameters()).is_cuda', next(problem.parameters()).is_cuda)
            # print('batch_feature.is_cuda', batch_feature.is_cuda)
            # print('solutions.is_cuda', solutions.is_cuda)
            # print('action.is_cuda', action.is_cuda)
            ...
            sol_times_traj.append((r_t, sol.cpu().detach().numpy()))
            # print('sol[0] for sol in sol_times_traj', [sol[0] for sol in sol_times_traj])
    ...
    def start_training(self, problem, val_dataset, train_dataset_class, tb_logger):
        if self.opts.distributed:
            mp.spawn(train, nprocs=self.opts.world_size, args=(problem, self, val_dataset, tb_logger))
        else:
            train(0, problem, self, val_dataset, train_dataset_class, tb_logger)


def train(rank, problem, agent, val_dataset, train_dataset_RA, tb_logger):
    opts = agent.opts
    ...
    for epoch in range(opts.epoch_start, opts.epoch_end):
        ...
        if rank == 0 and not opts.distributed:
            print('saving model')
            print('opts.no_saving', opts.no_saving)
            print('opts.no_saving', opts.no_saving)
            print('opts.checkpoint_epochs', opts.checkpoint_epochs)
            print('epoch % opts.checkpoint_epochs == 0', epoch % opts.checkpoint_epochs == 0)
...
def train_batch(
    ...
    while t < T:
        ...
        while t - t_s < n_step and not (t == T):
            ...
            # state transient
            solution, rewards, obj, solving_state, sol_clone = problem.step(batch, solution, action, obj, solving_state)
            # print('sol_clone.size()', sol_clone.size())
