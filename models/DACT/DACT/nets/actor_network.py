...
# changes: added "device" argument in init
...
class Actor(nn.Module):
    ...
    def __init__(self,
        ...
        self.decoder = MultiHeadDecoder(n_heads = self.n_heads_decoder,
                                        ...
                                        embed_dim = self.embedding_dim).to(self.device)
    ...
    def forward(self, problem, x_in, solution, exchange, do_sample = False, fixed_action = None, require_entropy = False, to_critic = False, only_critic  = False):
        ...
        if problem.NAME == 'cvrp':
            ...
            # print('solution.is_cuda', solution.is_cuda)
            # print('contex.is_cuda', contex.is_cuda)
            # print('next(self.embedder.parameters()).is_cuda', next(self.embedder.parameters()).is_cuda)
