...
from models.AM.AM.nets.graph_encoder import GraphAttentionEncoder
from models.AM.AM.utils.beam_search import CachedLookup
from models.AM.AM.utils.functions import sample_many
from models.AM.AM.utils.tensor_functions import compute_in_batches
from models.AM.AM.utils.log_utils import log_values
...
class AttentionModel(BaseModel):
    ...
    def train_model(self, train_dataset, val_dataset, ckpt_save_path, opts, tb_logger, coords_distribution,
                    sampling_args, generator_args):

        print('sampling_args', sampling_args)
        print('opts.batch_size', opts.batch_size)
        optimizer, baseline, lr_scheduler = self.train_prep(distribution=coords_distribution,
                                                            sample_args=sampling_args, generator_args=generator_args)
        epoch_rewards = []
        for epoch in range(opts.epoch_start, opts.epoch_start + opts.n_epochs):
            train_data_samples = train_dataset.sample(sample_size=opts.epoch_size).data_transformed
    ...
    def train_prep(self, model_data=None, resume_path=None, distribution=None, sample_args=dict, generator_args=dict):
        ...
        if self.train_opts.baseline == 'exponential':
            ...
            baseline = RolloutBaseline(self, self.problem.NAME, self.train_opts, distribution=distribution,
                                       sample_args=sample_args, generator_args=generator_args)
        else:
