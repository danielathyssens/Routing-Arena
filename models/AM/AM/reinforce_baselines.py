...
from models.AM.nets.attention_model import set_decode_type
from models.AM.utils import move_to
from models.AM.utils import load_problem
...
class RolloutBaseline(Baseline):
    ...
    def __init__(self, model, problem, opts, epoch=0, distribution=None, sample_args=None, generator_args=None):
        super(Baseline, self).__init__()

        # sample_size = 100, graph_size = 20, k = 4, cap = 30, max_cap_factor = 1.0, n_depots = 1
        ...
        print('sample_args', sample_args)
        self.graph_size = sample_args['graph_size'] # graph_size
        print('self.graph_size', self.graph_size)
        self.num_samples = sample_args['sample_size']
        self.distribution = distribution
        if self.graph_size is not None and self.num_samples is not None and self.distribution is not None:
            # graph_size, num_samples, distribution, _, _ = **kwargs
            # print('graph_size, num_samples, distribution', graph_size, num_samples, distribution)
            self.val_ds = CVRPDataset(transform_func=make_cvrp_instance,
                                      graph_size=self.graph_size,
                                      sampling_args=sample_args,
                                      generator_args=generator_args)
                                      # device=torch.device("cpu"))

            # print('self.val_ds.graph_size', self.val_ds.graph_size)
            # print('self.val_ds.sampling_args', self.val_ds.sampling_args)
            # print('self.val_ds.data_transformed[0]', self.val_ds.data_transformed[0])
            # print('self.val_ds.data[0]', self.val_ds.data[0])
            # print('self.val_ds[0]', self.val_ds[0])
    ...
    def _update_model(self, model, epoch, dataset=None):
        ...
        if dataset is None:
            self.dataset = self.val_ds.sample(sample_size=self.num_samples,
                                              graph_size=self.graph_size,
                                              distribution=self.distribution).data_transformed
    ...
    def epoch_callback(self, model, epoch, **gen_args):
        """
