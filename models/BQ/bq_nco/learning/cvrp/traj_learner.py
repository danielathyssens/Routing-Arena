...
from models.BQ.bq_nco.learning.cvrp.decoding import decode
from models.BQ.bq_nco.utils.misc import do_lr_decay, EpochMetrics, get_opt_gap
...
class TrajectoryLearner:
    ...
    def val_test(self, what="test"):
        ...
        with torch.no_grad():
            ...
            for batch_num, data in enumerate(dataloader):
                val_test_metrics, results = self.get_minibatch_val_test_metrics(data)
                epoch_metrics.update(val_test_metrics)
                if batch_num == DEBUG_NUM_BATCHES and self.debug:
                    break
                tours.append(results["tours"])
    ...
    def get_minibatch_val_test_metrics(self, data):
        ...
        decoding_metrics = {}
        # changed --> output tours -->make tours=True
        predicted_tour_lens, predicted_tours = decode(node_coords, dist_matrices, demands, capacities,
                                                      self.net, self.beam_size, self.knns, make_tours=True)

        opt_gap = get_opt_gap(predicted_tour_lens, tour_lens)
        decoding_metrics.update({'opt_gap': opt_gap})
        res["tours"] = predicted_tours  # already list
        res["costs"] = predicted_tour_lens.detach().cpu().numpy()

        return {**metrics, **decoding_metrics}, res
