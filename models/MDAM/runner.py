...
from models.runner_utils import merge_sols, print_summary_stats, eval_inference, get_time_limit
...
class Runner(BaseConstructionRunner):
    ...
    def run(self):
        ...
        if self.cfg.run_type in ['train', 'debug']:
            self.setup(compatible_problems=DATA_CLASS, data_transformation=prep_data_MDAM)
            self.train()
