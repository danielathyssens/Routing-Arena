...
def get_lkh_executable(url="http://www.akira.ruc.dk/~keld/research/LKH-3/LKH-3.0.4.tgz"):
    ...
    cwd = os.path.abspath(os.path.join("problems", "cvrp", "lkh"))
...
def solve_lkh(executable, depot, loc, demand, capacity):
    with tempfile.TemporaryDirectory() as tempdir:
        problem_filename = os.path.join(tempdir, "problem.cvrp")
...
if __name__ == "__main__":
    ...
    for dataset_path in opts.datasets:
        ...
        if opts.o is None:
            results_dir = os.path.join(opts.results_dir, "cvrp", dataset_basename)
