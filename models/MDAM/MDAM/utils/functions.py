import sys
sys.path.insert(0, '/home/thyssens/git/Research/L2O/routing-arena/models/MDAM/MDAM')
...
import pickle
from models.MDAM.MDAM import nets

def load_problem(name):
    from models.MDAM.MDAM.problems import TSP, CVRP, SDVRP, OP, PCTSPDet, PCTSPStoch
    #from models.MDAM.MDAM.
    problem = {
        'tsp': TSP,
        'cvrp': CVRP,
        'sdvrp': SDVRP,
...
def torch_load_cpu(load_path):
    # unpickler = pickle.Unpickler(load_path)
...
def load_model_search_(path, device, epoch=None):
    from models.MDAM.MDAM.nets.model_search import AttentionModel
    ...
    model = _load(model, model_filename)
    return model
...
def _load(model, state_dict_path):
    ...
    if state_dict_path is not None:
        # Load the model parameters from a saved state
        load_optimizer_state_dict = None
        print(f'  [*] Loading AM model for Evaluation from {state_dict_path} for {model.problem.NAME}')
