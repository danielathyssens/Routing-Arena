...
from models.MDAM.MDAM.train import rollout, get_inner_model
...
class RolloutBaseline(Baseline):
    ...
    def __init__(self, model, problem, opts, device, epoch=0, graph_size=None, num_samples=None, distribution=None):
        super(Baseline, self).__init__()
        ...
        # init val dataset (to sample from) # init in cpu and put batch later on gpu
    ...
    def _update_model(self, model, epoch, dataset=None):
        ...
        self.bl_vals = rollout(self.model, self.dataset, self.opts, self.device).cpu().numpy()
    ...
    def wrap_dataset(self, dataset):
        ...
        return BaselineDataset(dataset, rollout(self.model, dataset, self.opts, self.device).view(-1, 1))
    ...
    def epoch_callback(self, model, epoch):
        ...
        candidate_vals = rollout(model, self.dataset, self.opts, self.device).cpu().numpy()
