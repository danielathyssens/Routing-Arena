...
from models.NeuroLKH.NeuroLKH.CVRP_test import (
    write_candidate,
    read_feat,
)
    ...
    if method == "NeuroLKH":
        ...
        with Pool(num_workers) as pool:
            feats = list(tqdm.tqdm(pool.imap(method_wrapper, [
                ("FeatGen", dataset_name, dataset[i], str(i), max_nodes, lkh_exe_path)
            ...
            with Pool(num_workers) as pool:
                results = list(tqdm.tqdm(pool.imap(method_wrapper, [
                    ("NeuroLKH", dataset_name, dataset[i], data[i],
                     str(data[i].instance_id), candidate[i], n_nodes_extend[i],
                     rerun, max_trials, TIME_LIMIT, lkh_exe_path)
                    for i in range(len(dataset))
                ]), total=len(dataset)))
        else:
            with Pool(num_workers) as pool:
                results = list(tqdm.tqdm(pool.imap(method_wrapper, [
                    ("NeuroLKH", dataset_name, dataset[i], data[i],
                     str(data[i].instance_id), candidate[i], n_nodes_extend[i],
                     rerun, max_trials, data[i].time_limit, lkh_exe_path)
                    for i in range(len(dataset))
                ]), total=len(dataset)))
    else:
        assert method == "LKH"
        ...
        if num_workers <= 1:
            if TIME_LIMIT is not None:
                results = list(tqdm.tqdm([
                    method_wrapper(("LKH", dataset_name, dataset[i], data[i], str(data[i].instance_id),
                                    rerun, max_trials, TIME_LIMIT, lkh_exe_path))
                    for i in range(len(dataset))
                ], total=len(dataset)))
            else:
            ...
            if TIME_LIMIT is not None:
                with Pool(num_workers) as pool:
                    results = list(tqdm.tqdm(pool.imap(method_wrapper, [
                        ("LKH", dataset_name, dataset[i], data[i], str(data[i].instance_id),
                         rerun, max_trials, TIME_LIMIT, lkh_exe_path)
                        for i in range(len(dataset))
                    ]), total=len(dataset)))
            else:
